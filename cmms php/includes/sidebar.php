    ...
    <div class="mt-auto p-4 lg:p-8 border-t border-slate-700/50 w-full bg-medical-surface/50 backdrop-blur-md">
        <div class="flex items-center gap-4">
            ...
            <div class="hidden lg:block overflow-hidden flex-1">
                <p class="text-xs font-bold text-white leading-none truncate"><?= $_SESSION['user']['name'] ?? 'Usuario' ?></p>
                <p class="text-[10px] text-medical-blue uppercase font-black tracking-widest mt-1.5 truncate">
                    <?= $_SESSION['user']['role'] ?? 'Demo' ?></p>
            </div>
            <a href="?page=login&action=logout" class="text-slate-500 hover:text-red-500 transition-colors" title="Cerrar Sesión">
