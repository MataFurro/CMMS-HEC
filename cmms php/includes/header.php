...
// includes/header.php
?>
<header class="h-16 lg:h-20 border-b border-[var(--border-color)] bg-medical-surface/80 backdrop-blur-md sticky top-0 z-40 px-6 lg:px-10 flex items-center justify-between ml-20 lg:ml-0" x-data="{ showNotifications: false, showSettings: false }">
    <div class="hidden md:flex items-center bg-medical-surface border border-[var(--border-color)] rounded-lg px-4 py-1.5 group focus-within:border-medical-blue/50 transition-all">
        ...
        <input
            class="bg-transparent border-none focus:ring-0 text-xs w-48 lg:w-72 placeholder:text-text-muted/60 text-text-main outline-none"
    ...
    <div class="flex items-center gap-3 lg:gap-5">
        ...
        <button
            ...
            class="p-2 rounded-lg bg-medical-surface border border-[var(--border-color)] text-text-muted hover:text-medical-blue transition-all active:scale-95 flex items-center justify-center"
        ...
        <div class="relative">
            <button
                ...
                class="p-2 rounded-lg bg-medical-surface border border-[var(--border-color)] text-text-muted relative hover:text-medical-blue transition-all active:scale-95">
            ...
            <div x-show="showNotifications"
                ...
                class="absolute right-0 top-full mt-2 w-80 bg-medical-surface border border-[var(--border-color)] rounded-xl shadow-2xl z-50 overflow-hidden"
                style="display: none;">
        ...
        <div class="h-6 w-px bg-[var(--border-color)] hidden lg:block"></div>

        <!-- Settings Dropdown (Alpine.js) -->
        <div class="relative hidden lg:block">
            <button
                @click="showSettings = !showSettings; showNotifications = false"
                class="p-2 rounded-lg bg-medical-surface border border-[var(--border-color)] text-text-muted hover:text-medical-blue transition-all active:scale-95">
                <span class="material-symbols-outlined text-xl">settings</span>
            </button>

            <div x-show="showSettings"
                @click.away="showSettings = false"
                class="absolute right-0 top-full mt-2 w-48 bg-medical-surface border border-[var(--border-color)] rounded-xl shadow-2xl z-50 overflow-hidden"
                style="display: none;">
                ...
                <div class="p-1">
                    <button class="w-full text-left px-3 py-2 text-xs font-medium text-text-muted hover:text-text-main hover:bg-[var(--panel-dark)] rounded-lg flex items-center gap-2 transition-colors">
