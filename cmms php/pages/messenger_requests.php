...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <?php if (isset($_GET['success'])): ?>
        <div
            class="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-2xl flex items-center gap-4 text-emerald-500 shadow-xl shadow-emerald-500/5 animate-in slide-in-from-top-4 duration-300">
    ...
    <div class="card-glass overflow-hidden shadow-2xl border border-white/5">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr
                    class="bg-white/5 border-b border-white/10 uppercase text-[10px] font-black tracking-widest text-slate-500">
                ...
                <?php foreach ($requests as $r): ?>
                    <tr class="hover:bg-white/5 transition-all text-sm border-b border-white/5">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="text-medical-blue text-xs font-mono">
                                <?= htmlspecialchars($r['serie']) ?>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-slate-400 line-clamp-2 max-w-xs"
                            title="<?= htmlspecialchars($r['texto']) ?>">
                            <?= htmlspecialchars($r['texto']) ?>
                        </td>
                        <td class="px-6 py-4">
                            <?php if ($r['imagen_path']): ?>
                                <img src="../API Mail/uploads/<?= $r['imagen_path'] ?>"
                                    class="w-12 h-12 rounded-lg object-cover border border-white/10 hover:scale-150 transition-transform cursor-pointer">
                            <?php else: ?>
                        ...
                        <td class="px-6 py-4">
                            <span
                                class="px-2 py-1 rounded-full text-[10px] font-black uppercase bg-amber-500/10 text-amber-500 border border-amber-500/20">
                                <?= $r['status'] ?>
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex justify-center gap-2">
                                <form method="POST"
                                    onsubmit="return confirm('¿Crear Orden de Trabajo para esta solicitud?');">
