...
<div class="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div class="flex items-center justify-between">
        ...
        <a href="?page=work_orders" class="p-2.5 rounded-xl bg-panel-dark border border-border-dark text-text-muted hover:text-text-main transition-all flex items-center gap-2 text-xs font-bold uppercase tracking-wider">
    ...
    <form method="POST"
        ...
        <div class="grid grid-cols-1 gap-8 relative z-10">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-text-muted uppercase tracking-wider">Tipo de Orden</label>
                        <select required name="type" x-model="otType" class="w-full bg-medical-dark/50 border border-border-dark rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-medical-blue/20 focus:border-medical-blue outline-none transition-all text-text-main appearance-none">
                    ...
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-text-muted uppercase tracking-wider">Prioridad</label>
                        <select required name="priority" class="w-full bg-medical-dark/50 border border-border-dark rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-medical-blue/20 focus:border-medical-blue outline-none transition-all text-text-main appearance-none">
                ...
                <div x-transition class="space-y-2 pt-4 border-t border-border-dark/30">
                    ...
                    <select name="checklist_template" class="w-full bg-medical-dark/50 border border-border-dark rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-medical-blue/20 focus:border-medical-blue outline-none transition-all text-text-main appearance-none">
                        <option value="">-- Seleccionar Plantilla --</option>
                        <?php foreach (listChecklistTemplates() as $key => $label): ?>
                            <option value="<?= $key ?>"><?= $label ?></option>
                        <?php endforeach; ?>
                    </select>
                    <p class="text-[10px] text-text-muted italic flex items-center gap-1">
                ...
                <div class="space-y-2">
                    <label class="text-xs font-bold text-text-muted uppercase tracking-wider">Descripción del Problema / Solicitud</label>
                    <textarea required name="description" class="w-full bg-medical-dark/50 border border-border-dark rounded-xl p-4 text-sm focus:ring-2 focus:ring-medical-blue/20 focus:border-medical-blue outline-none transition-all min-h-[120px] text-text-main placeholder:text-text-muted/40" placeholder="Describa la falla, código de error o motivo del mantenimiento..."><?= htmlspecialchars($pre_description) ?></textarea>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-text-muted uppercase tracking-wider">Solicitante</label>
        ...
        <div class="pt-8 border-t border-border-dark/50 flex justify-end gap-4">
            <a href="?page=work_orders" class="px-8 py-3 rounded-2xl text-text-muted hover:text-text-main hover:bg-panel-dark transition-all text-sm font-bold uppercase tracking-wider text-center">
                Cancelar
            </a>
